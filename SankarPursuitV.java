 ...
 * Graphics Pd. 1
 */
public class SankarPursuitV extends Canvas{
	...
	public static void main(String[] args) {
		...
		SankarPursuitV v = new SankarPursuitV();
	...
	public void paint(Graphics g){
		...
		for (int i=0; i<25; i++){
			int fade =255*i/25;
			g.setColor(new Color(255,0,100,fade));//set the new color
		...
		for (int i=0; i<25; i++){	
			int fade = 255*i/25;
			g.setColor(new Color(255,0,255-fade,fade));//set color
		...
		//set color to orange
				...
				for (int j=0; j<25; j++){
					int fade = 255*j/25;
					g.setColor(new Color(255,255-fade,255-fade,fade));//set color
		...
		g.drawString(s,getWidth()/2-getFontMetrics(f).stringWidth(s)/2,getFontMetrics(f).getMaxAscent());
	}//end paint method
